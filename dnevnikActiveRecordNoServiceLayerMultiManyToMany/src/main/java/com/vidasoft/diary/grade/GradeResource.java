...
@RequestScoped
@Path(GRADE_RESOURCE_PATH)
public class GradeResource {
    public static final String GRADE_RESOURCE_PATH = "grade";
    ...
    */
    ...
    @Transactional
    public Response teacherWritesGradePerStudentPerSubject(@QueryParam("studentId") @NotNull @Positive Long stId,
                                                         @QueryParam("subjectId") @NotNull @Positive Long subjId,
                                                         @QueryParam("teacherId") @NotNull @Positive Long teacherId,
                                                         @Valid @NotNull GradeDTO gradeDTO) {
        ...
        Grade grade = new Grade(gradeDTO.grade, student, clazz, subject, teacher);
        grade.persist();
        GradeDTO gradeSavedDTO = new GradeDTO(grade);

        return Response.created(URI.create(String.format("/%s/%d", GRADE_RESOURCE_PATH, gradeSavedDTO.id))).entity(gradeSavedDTO).build();
    }

//    @GET()
//    @Produces(MediaType.APPLICATION_JSON)
//    @Path("/{studentId}/{subjectId}")
//    public Response getGradeByGradeId() {}


    @GET()
    @Produces(MediaType.APPLICATION_JSON)
    public Response getAllGradesPerStudentPerSubject(@QueryParam("studentId") @NotNull @Positive Long stId,
                                                    @QueryParam("subjectId") @NotNull @Positive Long subjId,
        ...
        List<Grade> allGradesFromAStudentAndASubjectOnePage =
                Grade.<Grade>find("student = ?1 AND subject = ?2 AND clazz = ?3", student, subject, clazz).page(page -1, size).list();
        List<GradeDTO> gradeDTOS = allGradesFromAStudentAndASubjectOnePage.stream().map(grd -> new GradeDTO(grd)).collect(Collectors.toList());
